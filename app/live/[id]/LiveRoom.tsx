...
export default function LiveRoom({ streamId }: { streamId?: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Not logged in → public preview + gate (no LiveKit)
      if (!meta?.userId) {
        setAuthRequired(true);
        try {
          const res = await fetch(`/api/live/preview?id=${id}`, {
            cache: 'no-store',
          });
          const data = await res.json().catch(() => ({}));
          if (res.ok && data.stream) {
            setStream((s) => s || (data.stream as StreamRow));
          ...
          if (!stream) setError('Could not load live');
        }
        setLoading(false);
        return;
      }
  ...
  // Guest gate — no adult stream until login (LoyalFans-style)
  if (authRequired && !userId) {
    const gateName =
      creator?.display_name ||
      (creator?.username ? `@${creator.username}` : 'Creator');
    const nextPath = `/live/${id}`;
    const loginHref = `/login?next=${encodeURIComponent(nextPath)}`;
    const signupHref = `/onboarding?next=${encodeURIComponent(nextPath)}`;
    const isEnded =
      liveStatus === 'ended' || stream?.status === 'ended';
        ...
        <div className="relative flex-1 flex flex-col items-center justify-center px-4 py-10 overflow-hidden">
          ...
          <div className="absolute inset-0">
            {stream?.thumbnail_url ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-zinc-900 via-zinc-950 to-pink-950/40" />
            )}
          ...
          <div className="relative z-10 w-full max-w-md">
            ...
            <div className="bg-zinc-900/90 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl shadow-pink-950/20 backdrop-blur-xl">
              <div className="relative aspect-video bg-zinc-950">
                {stream?.thumbnail_url ? (
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-pink-900/30 to-zinc-900" />
                )}
                ...
                {!isEnded && (
                  <span className="absolute top-3 left-3 bg-red-600 text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1 shadow">
              ...
              <div className="p-6 text-center">
                <div className="flex justify-center -mt-12 mb-3 relative z-10">
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-pink-500 to-rose-500 p-[3px] shadow-lg">
                    <div className="w-full h-full rounded-full bg-zinc-900 overflow-hidden flex items-center justify-center">
                      {creator?.avatar_url ? (
                        ...
                        <span className="text-2xl font-bold text-pink-300">
                          {gateName.charAt(0).toUpperCase()}
                        </span>
                      )}
                    </div>
                  </div>
                </div>
                <h1 className="text-xl font-bold text-white mb-1">{gateName}</h1>
                <p className="text-sm text-zinc-400 mb-1 line-clamp-2">
                  {stream?.title || 'Live stream'}
                </p>
                {isEnded ? (
                  <p className="text-sm text-zinc-500 mb-6">This live has ended</p>
                ) : (
                  <p className="text-sm text-zinc-500 mb-6">
                    Log in to watch · 18+ only
                  </p>
                )}
                ...
                {!isEnded ? (
                  <div className="space-y-3">
                    <Link
                      href={loginHref}
                      className="block w-full bg-gradient-to-r from-pink-600 to-rose-500 hover:opacity-90 text-white font-semibold py-3.5 rounded-xl transition text-center"
                    >
                      Log in to watch
                    </Link>
                    <Link
                      href={signupHref}
                      className="block w-full border border-zinc-700 hover:border-pink-500/50 text-zinc-200 font-medium py-3.5 rounded-xl transition text-center"
                    >
                      Create account
                    </Link>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <Link
                      href="/live"
                      className="block w-full border border-zinc-700 text-zinc-300 py-3 rounded-xl text-center text-sm"
                    >
                      Browse lives
                    </Link>
                  </div>
                ...
                <p className="text-[11px] text-zinc-600 mt-5 leading-relaxed">
                  World Of Dommes is an adults-only platform. You must be 18 or
                  over and signed in to watch live streams.
                </p>
              </div>
            </div>

            <p className="text-center mt-6">
              <Link href="/" className="text-sm text-zinc-500 hover:text-pink-400">
                ← Home
              </Link>
            </p>
          </div>
